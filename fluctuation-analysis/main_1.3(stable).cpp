...
void click_event(int event, int x, int y, int flags, void *data)
{
    ...
    if (event == EVENT_LBUTTONDOWN)
    {
        ...
        putText(imageCopy, to_string(x) + "," + to_string(y), Point(x - 25, y), FONT_HERSHEY_SIMPLEX, 1, CV_RGB(255, 0, 0), 1);
...
void on_change(int val, void *data)
{
    ...
    if (radius_outer < radius_inner)
        putText(imageCopy, "outer radius is smaller than inner radius", Point(params[0], params[1]), FONT_HERSHEY_SIMPLEX, 0.5, CV_RGB(255, 0, 0), 1);
    imshow(windowName, imageCopy);
...
int main(int argc, char **argv)
{
    ...
    for (int i = first_frame; i < final_frame + 1; i++)
    {
        ...
        else
        {
            ...
            while ((deviation(radius_vec) > 7) && (stage_mov == 1))
    ...
    cout << "Time Correlation exececution time is: " << duration_2.count() << " ms" << endl;
    cout << "\n"
         << "Program ran successfully!!" << endl;
