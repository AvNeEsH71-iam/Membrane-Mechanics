...
void click_event(int event, int x, int y, int flags, void *data)
{
    ...
    if (event == EVENT_LBUTTONDOWN)
    {
        ...
        putText(imageCopy, to_string(x) + "," + to_string(y), Point(x - 25, y), FONT_HERSHEY_SIMPLEX, 1, CV_RGB(255, 0, 0), 2);
        ...
        if (params[3] > 0 && params[3] < params[2])
        {
            circle(imageCopy, Point(x, y), params[2] - params[3], CV_RGB(0, 0, 255));
            circle(imageCopy, Point(x, y), params[2] + params[3], CV_RGB(0, 0, 255));
        }
...
void on_change(int val, void *data)
{
    ...
    // get slider values
    int mean_radius = getTrackbarPos("mean radius", windowName);
    int annulus = getTrackbarPos("annulus", windowName);

    // store slide values
    if (mean_radius != 0)
        params[2] = mean_radius;
    if (annulus != 0 && mean_radius > annulus)
        params[3] = annulus;
    if (annulus > mean_radius)
        putText(imageCopy, "mean radius is smaller than annulus", Point(10, 10), FONT_HERSHEY_SIMPLEX, 0.5, CV_RGB(255, 0, 0), 2);
    imshow(windowName, imageCopy);

    // show circle with user given radii on copy of image
    circle(imageCopy, Point(params[0], params[1]), mean_radius, CV_RGB(0, 255, 0));
    if (annulus > 0 && annulus < mean_radius)
    {
        circle(imageCopy, Point(params[0], params[1]), mean_radius - annulus, CV_RGB(0, 0, 255));
        circle(imageCopy, Point(params[0], params[1]), mean_radius + annulus, CV_RGB(0, 0, 255));
    }
...
void contour_detection(Mat *img, vector<int> &contour_X, vector<int> &contour_Y, vector<double> &radius_vec, size_t m, size_t n, int boundary_type)
{
    ...
    vector<double> radius = linspaceFunc(params[2] - params[3], params[2] + params[3], n);
    ...
    else
    {
        for (int i = 0; i < m; i++)
        {
            ...
            for (int j = 1; j < n - 1; j++)
            {
                if ((polar_img[i][j + 1] - polar_img[i][j - 1]) / (double)2 >= max)
                {
                    max = (polar_img[i][j + 1] - polar_img[i][j - 1]) / (double)2;
                    r = radius[j];
                }
            }
...
void user_input(Mat img, string filename, string first_frame_directory)
{
    ...
    createTrackbar(
        "mean radius", windowName, 0, min(img.rows, img.cols), on_change, &filename);
    createTrackbar(
        "annulus", windowName, 0, min(img.rows, img.cols), on_change, &filename);
...
int main(int argc, char **argv)
{
    ...
    // save contour image
    int contour_store;
    cout << "Enter '1' if you want to store contour images else enter '0'" << endl;
    cin >> contour_store;

    string savelocation;
    if (contour_store == 1)
    {
        cout << "Enter directory to store contour images: " << endl;
        cin >> savelocation;
    }
    ...
    for (int i = first_frame; i < final_frame + 1; i++)
    {
        ...
        else
        {
            ...
            // contour detection
            contour_detection(&img, contour_X, contour_Y, radius_vec, m, n, boundary_type);
            ...
            while ((deviation(radius_vec) > 4) && (stage_mov == 1))
            ...
            {
                circle(img, Point(params[0], params[1]), 2, CV_RGB(255, 255, 255), -1);

                if (boundary_type == 'd')
                {
                    circle(img, Point(contour_X[j], contour_Y[j]), 0, CV_RGB(255, 0, 0), -1);
                }
                else
                {
                    circle(img, Point(contour_X[j], contour_Y[j]), 0, CV_RGB(0, 0, 255), -1);
                }
            }
            ...
            myfile.open("mean_radius.txt", ios_base::app);
            myfile << r_avg << "\n";
            myfile.close();
            ...
            cout << "Contour detection progress is: ";
            cout << (i / (double)final_frame) * 100 << "%";
            cout.flush();
    ...
    cout << "Contour Detection exececution time is: " << duration.count() << " s" << endl;

    start = high_resolution_clock::now();

    time_autocorr(l_max, frame_count);

    stop = high_resolution_clock::now();
    auto duration_2 = duration_cast<milliseconds>(stop - start);

    cout << "Time Auto Correlation exececution time is: " << duration_2.count() << " ms" << endl;

    start = high_resolution_clock::now();

    time_crosscorr(l_max, frame_count);

    stop = high_resolution_clock::now();
    auto duration_3 = duration_cast<milliseconds>(stop - start);

    cout << "Time Cross Correlation exececution time is: " << duration_3.count() << " ms" << endl;
    cout << "\n"
         << "Contour Detection and Decomposition program ran successfully!!\n"
         << endl;
    cout << "=================================================\n"
         << endl;
